    ...
    <WidgetCard title="Explainable AI Analysis" icon={<Lightbulb size={16} />}>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 flex flex-col gap-6">
          <div>
            <h4 className="text-lg font-bold text-[var(--text-primary)] mb-3">What influenced this prediction?</h4>
            <p className="text-sm text-[var(--text-secondary)] mb-4 leading-relaxed">
              The model classified this segment as <span className="font-semibold text-[var(--text-primary)]">{isNormal ? 'Normal Activity' : 'Seizure'}</span> because:
            </p>
            <ul className="space-y-3">
              {topFeature && (
                <li className="flex gap-2 text-sm text-[var(--text-primary)] items-start">
          ...
          <div className="p-4 bg-[var(--bg-3)] rounded-xl border border-[var(--bg-4)] mt-auto">
            <div className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider mb-2">Most Influential Feature</div>
            <div className="text-sm font-semibold text-[var(--text-primary)]">{topFeature?.featureName}</div>
            <div className="text-xs text-[var(--text-secondary)] mt-1 font-mono">SHAP Value: {topFeature?.value > 0 ? '+' : ''}{topFeature?.value.toFixed(4)}</div>
          </div>
        </div>

        {/* Right: Existing SHAP Plot */}
        <div className="lg:col-span-2 bg-[var(--bg-3)] p-4 rounded-xl border border-[var(--bg-4)] flex items-center justify-center">
          <ShapWaterfall 
