...
from research.experiments.eeg_specific.eeg_models import build_eeg_model
...
class MMapEpochDataset(Dataset):
    """
    Zero-RAM dataset reading from memory-mapped positive and epoch negative arrays.
    """
    def __init__(self, pos_x: np.ndarray, pos_y: np.ndarray, neg_x: np.ndarray, neg_y: np.ndarray, seed: int = 42):
        ...
        self.indices = rng.permutation(self.total_samples)
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if real_idx < self.n_pos:
            ...
            neg_offset = real_idx - self.n_pos
            x = self.neg_x[neg_offset]
            y = self.neg_y[neg_offset]
...
def run_experiment(
    ...
    print("=" * 80)
    print(f"STARTING EXPERIMENT 2 RUN: {model_name.upper()}")
    print("=" * 80)
    ...
    if output_dir is None:
        out_dir = BASE_DIR / "research" / "experiments" / "eeg_specific" / model_name
    else:
    ...
    # 2. Memory-map pre-cached training windows
    print("\n[Step 2] Memory-mapping pre-cached training windows...")
    pos_x = np.load(CACHE_DIR / "pos_windows.npy", mmap_mode="r")
    pos_y = np.load(CACHE_DIR / "pos_labels.npy", mmap_mode="r")
    print(f"Memory-mapped positive windows: {pos_x.shape}")

    # 3. Model instantiation
    print(f"\n[Step 3] Initializing architecture: {model_name.upper()}...")
    model = build_eeg_model(model_name)
    model.to(device)
    n_params = model.get_num_parameters()
    print(f"Model Parameters: {n_params:,} (Limit: < 500,000)")
    ...
    # 4. Training loop over 3 epochs
    print(f"\n[Step 4] Training {model_name.upper()} for {epochs} epochs (batch_size={batch_size})...")
    training_start_time = time.time()

    for epoch in range(epochs):
        t_ep0 = time.time()
        neg_x = np.load(CACHE_DIR / f"neg_windows_ep{epoch}.npy", mmap_mode="r")
        neg_y = np.load(CACHE_DIR / f"neg_labels_ep{epoch}.npy", mmap_mode="r")

        dataset = MMapEpochDataset(pos_x, pos_y, neg_x, neg_y, seed=seed + epoch)
        ...
        del dataset, dataloader, neg_x, neg_y
        flush_memory()

        # Validation evaluation pass with bounded streaming
        print(f"  Epoch {epoch+1} training done in {t_tr1 - t_tr0:.1f}s. Evaluating validation cohort...")
        t_val0 = time.time()
        val_true, val_probs = evaluate_with_streaming(
            model=model,
            split_df=val_df,
            device=device,
            batch_size=64,
        ...
        print(f"Epoch {epoch+1}/{epochs} [{model_name.upper()}] ({ep_duration:.1f}s):")
        print(f"  Train Loss: {avg_train_loss:.5f} | Val AUPRC: {val_auprc:.5f} | Val AUROC: {val_auroc:.5f}")
    ...
    print("\n" + "=" * 80)
    print(f"[Step 5] Final Single-Pass Evaluation on LOCKED Test Cohort ({model_name.upper()})...")
    print("=" * 80)
    ...
    print(f"Test Set: {len(test_df):,} windows, {test_duration_hours:.2f} hours across {sorted(test_df['patient_id'].unique())}")

    t_inf0 = time.time()
    test_true, test_probs = evaluate_with_streaming(
        model=model,
        split_df=test_df,
        device=device,
        batch_size=64,
    ...
    t_mean_d, t_med_d, t_min_d, t_max_d, t_det_ev, t_tot_ev, _ = compute_event_delay_details(
        window_df=test_df,
        events_df=events_df,
        y_prob=test_probs,
        threshold=0.5
    ...
    dummy_win = torch.randn(1, 23, 1280).to(device)
    with torch.no_grad():
        for _ in range(10):
            _ = model(dummy_win)
    ...
    print(f"\n--- TEST METRICS SUMMARY [{model_name.upper()}] ---")
    ...
    # 6. Save outputs
    print(f"\n[Step 6] Serializing artifacts to {out_dir}...")
    ...
    config_dict = {
        "experiment": f"eeg_specific_{model_name}",
        "model_name": model_name,
        "input_channels": 23,
        "input_samples": 1280,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run EEG-specific model experiment")
    parser.add_argument("--model", type=str, required=True, choices=["eegnet", "shallowconvnet", "deepconvnet", "cnn1d"], help="Model name")
