...
- **Train/Val Overlap**: PASS (0)
- **Train/Test Overlap**: PASS (0)
- **Val/Test Overlap**: PASS (0)

**Found Partitions**:
- **Train**: chb04, chb09, chb11, chb12, chb13, chb14, chb15, chb16, chb17, chb18, chb19, chb20, chb21, chb22, chb23, chb24
- **Validation**: chb06, chb07, chb08, chb10
- **Test**: chb01, chb02, chb03, chb05
...
- **Validation Mismatches**: 739 windows (0.25% error rate against stored labels, likely due to stored predictions using `label_any_overlap` rather than 50%).
- **Test Mismatches**: 637 windows (0.29% error rate).
...
- **Total Events**: 25
- **Detected Events**: 15
- **Event Sensitivity**: 60.00%
- **Mean Detection Delay**: 6.07 seconds (calculated strictly as onset-to-onset)

## 12. Validation False Alarms/day
- **False-Positive Windows**: 4467
- **Merged False-Alarm Episodes**: 805
- **Monitoring Hours**: 203.82
- **FA/Day (Episodes)**: 94.79

## 13. Final Test Recalculation
- **Total Events**: 22
- **Detected Events**: 21 (95.45%)
- **Window Sensitivity**: 83.83%
- **Specificity**: 99.82%
- **AUROC**: 0.98970
- **AUPRC**: 0.80681
- **Mean Detection Delay (Onset)**: 5.57 seconds
- **False Positive Windows**: 399
- **Merged False-Alarm Episodes**: 80
- **FA/Day (Episodes)**: 12.56
*(Note: Historical documentation reported 62.66 FA/day by incorrectly substituting FP windows (399) into the FA episodes formula. The recalculated mathematical FA episodes rate is 12.56 FA/day).*

## 14. Missed Event Forensics
- **Patient**: chb01
- **Recording**: chb01_15
- **Event**: 2
- **Reason**: Probabilities never exceeded threshold during event (Max predicted probability was 0.481, which did not cross the 0.5 threshold).
...
- **Severity**: HIGH
- **Impact**: FA/day was artificially inflated in historical reports. True FA/day (episodes) is 12.56.
- **Recommended action**: Adopt episode-based merging for all future benchmarking.

**Problem**: Detection Delay End-of-Window Bias
- **Evidence**: Historical delay reported as ~10.57s. Actual onset-to-onset delay is 5.57s. The +5s offset indicates the historical calculation used the end of the first detected window.
- **Severity**: MEDIUM
...
- **Evidence**: 739 validation and 637 test labels in the stored CSV predictions conflict with a strict 50% overlap reconstruction, revealing the CSVs were generated with `label_any_overlap`.
- **Severity**: MEDIUM
