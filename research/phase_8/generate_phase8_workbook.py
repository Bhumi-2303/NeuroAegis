...
Output: research/phase_8/NeuroAegis_Final_Research_Results.xlsx
...
WORKBOOK_PATH = os.path.join(BASE_DIR, "research/phase_8/NeuroAegis_Final_Research_Results.xlsx")
FINAL_RESULTS_DIR = os.path.join(BASE_DIR, "research/phase_8/final_results")
...
def create_master_workbook():
    ...
    # 9. False_Alarms
    append_df_sheet("False_Alarms", os.path.join(BASE_DIR, "research/phase_7/results/false_alarm_results.csv"), "FALSE ALARM SUPPRESSION PROFILES ACROSS TEST PATIENTS")

    # 10. Detection_Delay
    append_df_sheet("Detection_Delay", os.path.join(BASE_DIR, "research/phase_7/results/detection_delay_results.csv"), "SEIZURE EVENT DETECTION DELAY DISTRIBUTION")
    ...
    # 12. Bootstrap_CI
    append_df_sheet("Bootstrap_CI", os.path.join(BASE_DIR, "research/phase_7/results/bootstrap_results.csv"), "PATIENT-CLUSTER & EVENT RESAMPLING BOOTSTRAP 95% CONFIDENCE INTERVALS (5,000 ITERATIONS)")
    ...
    # 15. XAI
    append_df_sheet("XAI", os.path.join(BASE_DIR, "research/phase_5/results/channel_attribution_summary.csv"), "EXPLAINABLE AI: 23-CHANNEL INTEGRATED GRADIENTS ATTRIBUTIONS & RANKINGS")
    ...
    fig_table = [
        ...
        ["Figure 4", "figure_04_spatial_graph_topology.png", "Spatial Graph Topology Adjacency (θ=0.30)", "research/phase_4a/frozen_graph_adjacency.csv", "300 DPI PNG"],
        ["Figure 5", "figure_05_ablation_performance.png", "Three-Model Ablation Performance Progression", "research/phase_7/results/model_comparison.csv", "300 DPI PNG"],
        ["Figure 6", "figure_06_final_confusion_matrix.png", "Model C Authoritative Test Confusion Matrix", "research/phase_4b/results/final_test_predictions.csv", "300 DPI PNG"],
        ...
        ["Figure 9", "figure_09_event_detection_timeline.png", "Seizure Event Detection Timeline (22 Events)", "research/phase_4b/results/final_test_event_results.csv", "300 DPI PNG"],
        ["Figure 10", "figure_10_patient_level_performance.png", "Patient-Level Sensitivity & False Alarm Distributions", "research/phase_4b/results/final_test_patient_results.csv", "300 DPI PNG"],
        ["Figure 11", "figure_11_false_alarm_comparison.png", "False Alarm Rate Suppression Across Models (-96.8%)", "research/phase_7/results/false_alarm_results.csv", "300 DPI PNG"],
        ["Figure 12", "figure_12_detection_delay_distribution.png", "Seizure Event Detection Delay Distribution & ECDF", "research/phase_4b/results/final_test_event_results.csv", "300 DPI PNG"],
        ["Figure 13", "figure_13_siena_zero_shot_performance.png", "Siena Zero-Shot Benchmark Subset Performance", "research/phase_6/results/siena_zero_shot_summary.json", "300 DPI PNG"],
        ["Figure 14", "figure_14_siena_adaptation_recovery.png", "Siena Post-Hoc Domain Calibration Recovery (+23.2%)", "research/phase_6/results/siena_adapted_summary.json", "300 DPI PNG"],
        ["Figure 15", "figure_15_xai_channel_attribution.png", "Top-12 Salient EEG Channels (Integrated Gradients)", "research/phase_5/results/channel_attribution_summary.csv", "300 DPI PNG"],
        ["Figure 16", "figure_16_xai_temporal_attribution.png", "Temporal Feature Attribution Profile (5.0s Window)", "research/phase_5/results/temporal_attribution_summary.csv", "300 DPI PNG"],
        ["Figure 17", "figure_17_xai_faithfulness.png", "Explainability Faithfulness: Insertion & Deletion Curves", "research/phase_5/results/insertion_deletion_results.csv", "300 DPI PNG"],
        ["Figure 18", "figure_18_complexity_vs_performance.png", "Model Complexity vs AUPRC Pareto-Efficiency", "research/phase_7/results/model_comparison.csv", "300 DPI PNG"]
    ]
    ...
    tables_list = [
        ...
        ["Table 1", "Dataset Characteristics", "Summary of CHB-MIT and Siena databases", "research/phase_8/final_results/authoritative_final_metrics.json"],
        ["Table 2", "CHB-MIT Patient Partition", "Patient-independent 16/4/4 disjoint split", "research/phase_8/final_results/reproducibility_manifest.json"],
        ["Table 3", "Architecture Comparison", "Structural dimensions of 1D-CNN, CNN-GNN, CNN-GNN-GRU", "research/phase_8/final_results/final_model_comparison.csv"],
        ["Table 4", "CHB-MIT Final Test Performance", "Authoritative test metrics across 219,909 windows", "research/phase_8/final_results/authoritative_final_metrics.csv"],
        ["Table 5", "Ablation Study", "False alarm suppression (-96.8%) and sensitivity restoration", "research/phase_8/final_results/final_ablation.csv"],
        ["Table 6", "Patient-Level Performance", "Individual metrics across test subjects chb01, chb02, chb03, chb05", "research/phase_8/final_results/final_patient_results.csv"],
        ["Table 7", "Event-Level Seizure Detection", "Detection status, delay, duration across 22 test events", "research/phase_8/final_results/final_event_results.csv"],
        ["Table 8", "Cross-Domain Generalization", "Source test vs Siena zero-shot benchmark subset", "research/phase_8/final_results/final_cross_domain.csv"],
        ["Table 9", "Domain Adaptation Recovery", "Post-hoc temperature and threshold calibration on PN12", "research/phase_8/final_results/final_cross_domain.csv"],
        ["Table 10", "Explainability Attributions", "Top channel and temporal attributions with faithfulness metrics", "research/phase_8/final_results/final_xai_results.csv"],
        ["Table 11", "Computational Complexity", "Parameters, latency (1.42ms), memory, and real-time capability", "research/phase_8/final_results/authoritative_final_metrics.csv"],
        ["Table 12", "Statistical Robustness", "Bootstrap 95% CIs, McNemar tests, and effect sizes", "research/phase_8/final_results/final_statistical_results.csv"],
        ["Table 13", "Limitations & Boundaries", "Formal documentation of research boundaries and future work", "research/phase_8/publication/final_limitations.md"]
    ]
